...
When reconstructed panels are stored outside this reproduction package, set `DCSI_PROJECT_ROOT` to the reconstructed project root before running analysis commands.
...
The relation-augmented DCSI scorecard is reproduced with:
...
The two-sided review and routine-monitoring triage scorecards are built from archived online predictions with:
...
The archived scorecards in `results/scorecards/` report held-out AUC differences, online lead-time differences, advisory-feature references, review-cost sensitivity, two-sided triage, paired bootstrap intervals, memory selection, decile closure, event timing, semantic constraint memory, shifted timing diagnostics, negative controls, and transfer checks.
