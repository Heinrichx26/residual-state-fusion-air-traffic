...
The archived scorecards in `results/scorecards/` report held-out AUC gains, strong advisory-feature references, paired bootstrap intervals, memory selection, decile closure, event timing, semantic constraint memory, shifted timing diagnostics, negative controls, and transfer checks.
